...
export const productMap = new Map<string, Product>(
  allProducts.filter(Boolean).map((p) => [p.id, p])
);

// Map Category ID -> Set of Product IDs
export const categoryProductIdsMap = new Map<string, Set<string>>(
  categories.map((c) => [c.id, new Set((c.products || []).filter(Boolean).map((p) => p.id))])
);

export const productCategoryIdMap = new Map<string, string>(
  categories.flatMap((c) => (c.products || []).filter(Boolean).map((p) => [p.id, c.id] as const))
);
